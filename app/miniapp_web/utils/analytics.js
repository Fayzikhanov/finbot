(function (global) {
  ...
  function buildExpenseDayStats(items, startIso, endIso) {
    ...
    var effectiveStart = startIso;
    var firstTxDate = "";
    (Array.isArray(items) ? items : []).forEach(function (row) {
      var dk = dayKeyFromTx(row);
      if (dk && (!firstTxDate || dk < firstTxDate)) firstTxDate = dk;
    });
    if (firstTxDate && firstTxDate > startIso) effectiveStart = firstTxDate;
    return {
      daily,
      maxDay,
      avgExpensePerDay: (buildTotals(items).expense || 0) / Math.max(1, diffDaysInclusive(effectiveStart, endIso)),
    };
  }
